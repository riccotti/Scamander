...
class linear_explanation:
    ...
    def give_explanation_tuple(self, x, upto=10):
        test_entry = self.short_factual
        out = self.recover_all_val(self.var)
        mask = np.abs(out - test_entry) > 0.001

        counterfactuals = list()
        counterfactual_instances = list()

        counterfactual = list()
        new_out = np.zeros(out.shape)
        for i in range(mask.size):
            if mask[i]:
                counterfactual.append((self.cox[i].name, out[i] * self.cox[i].scale))
                new_out[i] = out[i] * self.cox[i].scale
            else:
                new_out[i] = x[i]

        if len(counterfactual) > 0:
            counterfactuals.append(counterfactual)
            counterfactual_instances.append(new_out)
        ...
        return counterfactuals, counterfactual_instances
    ...
    def explain_entry_counterfactuals(self, entry, upto=10):
        self.set_factual(entry)
        self.build_structure()
        text = self.give_explanation_tuple(entry, upto)
        return text
