...
def experiment(cfe, bb, bb_dice, X_train, variable_features, features_names, variable_cont_features_names,
               ...
               variable_features_flag, known_train, search_diversity, metric, backend, nbr_exp):
    ...
    for nbr_train_perc in np.arange(0.2, 1.2, 0.2):
        ...
        for id_exp in range(nbr_exp):
            ...
            df = pd.DataFrame(data=X_train_sub, columns=features_names)
            ...
            print(datetime.datetime.now(), dataset, black_box, 'train size')
...
def main():
    ...
    filename_train_size = path_results + 'train_size_%s_%s_dice.csv' % (dataset, black_box)
    ...
    experiment('dice', bb, bb_dice, X_train, variable_features, features_names, variable_cont_features_names,
               ...
               filename_train_size, variable_features_flag, known_train, search_diversity, metric,
               backend, nbr_exp_per_train_size)
