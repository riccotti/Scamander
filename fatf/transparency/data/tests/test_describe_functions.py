...
def test_describe_numerical_array():
    ...
    array = np.array([(33, 22, np.nan, 11, np.nan, 4)],
                     dtype=[('a', int), ('b', int), ('c', np.float),
                            ('dimensionality', np.int32), ('e', np.float), ('f', int)])
...
def test_describe_categorical_array():
    ...
    # Structured row
    array = np.array([('aa', 'bb', 'abc', 'dimensionality', 'bb')],
                     dtype=[('a', 'U2'), ('b', 'U2'), ('c', 'U3'), ('dimensionality', 'U4'),
                            ('e', 'U2')])
    description = {'count': 5, 'unique': np.array(['aa', 'abc', 'bb', 'dimensionality']),
    ...
    array = np.array(['dimensionality', 'bb', 'abc', 'dimensionality', 'bb'])
    description = {'count': 5, 'unique': np.array(['abc', 'bb', 'dimensionality']),
...
def test_describe_array():
    ...
    # Testing arrays
    numerical_indices_struct = ['a', 'b', 'dimensionality']
    ...
    description_num = {'a': num_c0, 'b': num_c1, 'dimensionality': num_c2, 0: num_c0,
                       1: num_c1, 3: num_c2, 2: num_c2}
    description_num_nan = {'a': num_c0, 'b': num_c1_nan, 'dimensionality': num_c2,
                           0: num_c0, 1: num_c1_nan, 3: num_c2, 2: num_c2}
    description_num_nann = {'a': num_c0, 'b': num_c1_nann, 'dimensionality': num_c2,
    ...
    assert set(description) == set('dimensionality')
    for col_id, column_description in description.items():
        assert set(ftddf.NUMERICAL_KEYS) == set(column_description.keys())
        assert set(ftddf.NUMERICAL_KEYS) == set(
    ...
    with pytest.raises(IndexError) as exin:
        ftddf.describe_array(
            array_struct, include='a', exclude=['a', 7.5, 'b', 'x'])
    exin_message = str(exin.value)
    assert (exin_message.startswith(value_error_exclude_indices)
            and '{' in exin_message and '}' in exin_message
            and '7.5' in exin_message and 'x' in exin_message)
    ...
    # Include list
    description = ftddf.describe_array(
        array_struct, include=['a', 'dimensionality'], exclude='categorical')
    assert set(description) == set(['a', 'dimensionality'])
    for col_id, column_description in description.items():
        assert set(ftddf.NUMERICAL_KEYS) == set(column_description.keys())
        assert set(ftddf.NUMERICAL_KEYS) == set(
            description_num_nan[col_id].keys())
    ...
    assert set(description) == set(['a', 'b', 'dimensionality'])
    for col_id, column_description in description.items():
        assert set(ftddf.NUMERICAL_KEYS) == set(column_description.keys())
