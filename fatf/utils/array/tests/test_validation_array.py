...
def test_is_2d_array():
    ...
    flat_struct = [
        np.dtype([('num_points', NUMERICAL_STRUCTURED_ARRAY.dtype[0])]),
        np.dtype([('num_points', NUMERICAL_STRUCTURED_ARRAY.dtype[1])]),
        np.dtype([('num_points', NOT_NUMERICAL_STRUCTURED_ARRAY.dtype[0])]),
        np.dtype([('num_points', NOT_NUMERICAL_STRUCTURED_ARRAY.dtype[1])])]
    # yapf: enable
...
def test_is_structured_row():
    ...
    with pytest.raises(TypeError) as exin:
        fuav.is_structured_row(
            np.array((1., (1 + 1j)), dtype=[('num_points', '<f8'), ('c', '<c16')]))
    assert str(exin.value) == type_error
    ...
    with pytest.raises(TypeError) as exin:
        fuav.is_structured_row(
            np.array([(1., ), (2, ), (3, )], dtype=[('num_points', '<f8')]))
    assert str(exin.value) == type_error
    # Void types
