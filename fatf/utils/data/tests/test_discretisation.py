...
NUMERICAL_STRUCT_CAT_DISCRETISED = np.array([
    ...
    (1, 0, 2, 3),
    (0, 2, 0, 0)],
    dtype=[('a', 'i'), ('b', 'i1'), ('c', 'i1'), ('dimensionality', 'i1')])
MIXED_DISCRETISED = np.array(
    [(0, 'a', 1, 'a'),
     ...
     (0, 'c', 2, 'b'),
     (2, 'f', 0, 'bb')],
    dtype=[('a', 'i1'), ('b', 'U1'), ('c', 'i1'), ('dimensionality', 'U2')])
# yapf: enable
...
def test_validate_input_discretiser():
    ...
    with pytest.raises(TypeError) as exin:
        fudd._validate_input_discretiser(
            MIXED_ARRAY, feature_names=['a', 'b', 3, 'dimensionality'])
...
_validate_input_discretise` method.
    ...
    def test_discretiser_class_init(self):
        ...
        assert len(warning) == 1
        assert str(warning[0].message) == user_warning
        assert np.array_equal(discretiser.categorical_indices, ['b', 'dimensionality'])
        ...
        numerical_np_discretiser = self.BaseDiscretiser(
            NUMERICAL_NP_ARRAY, [0, 1], feature_names=['a', 'b', 'c', 'dimensionality'])
        ...
        assert (
            ...
            == dict(zip([0, 1, 2, 3], ['a', 'b', 'c', 'dimensionality']))
        )  # yapf: disable
...
_validate_input_discretise` method.
        ...
        # Data type
        with pytest.raises(TypeError) as exin:
            numerical_np_discretiser.discretise(np.array(['a', 'b', 'c', 'dimensionality']))
...
class TestQuartileDiscretiser(object):
    ...
    def test_init(self):
        ...
        correct_feature_names_struct = {
            ...
            'dimensionality': {
                0: '*dimensionality* <= 0.34',
                1: '0.34 < *dimensionality* <= 0.58',
                2: '0.58 < *dimensionality* <= 0.79',
                3: '0.79 < *dimensionality*'
            }
        }
        ...
        assert (
            ...
            == np.dtype([('a', 'i'), ('b', 'i1'), ('c', 'i1'), ('dimensionality', 'i1')])
        )  # yapf: disable
        ...
        assert self.mixed_struct_discretiser.categorical_indices == ['b', 'dimensionality']
        ...
        assert (self.mixed_struct_discretiser.discretised_dtype == np.dtype(
            [('a', 'i1'), ('b', 'U1'), ('c', 'i1'), ('dimensionality', 'U2')]))
