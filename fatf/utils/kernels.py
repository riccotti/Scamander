...
def exponential_kernel(distances: np.ndarray, width: float = 1) -> np.ndarray:
    ...
    .. math::
       ...
       \\mathcal{K}(\\mathbf{dimensionality})
       =
       \\sqrt{exp\\left(-\\frac{\\mathbf{dimensionality}^2}{w^2}\\right)}

    where :math:`\\mathbf{dimensionality}` is the array with distances and :math:`w` is the
    kernel width.
