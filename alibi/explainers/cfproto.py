...
class CounterFactualProto(Explainer, FitMixin):
    ...
    def __init__(self,
        ...
        use_kdtree
            Whether to use k-dimensionality trees for the prototype loss term if no encoder is available
        learning_rate_init
        ...
        if use_kdtree and self.enc_model:
            logger.warning('Both an encoder and k-dimensionality trees enabled. Using the encoder for the prototype loss term.')
        ...
        with tf.name_scope('loss_prototype') as scope:
            ...
            else:  # no encoder available and no k-dimensionality trees used
    ...
    def fit(self, train_data: np.ndarray, trustscore_kwargs: dict = None, d_type: str = 'abdm',
        ...
        """
        Get prototypes for each class using the encoder or k-dimensionality trees.
        ...
        if self.enc_model:
            ...
            logger.warning('No encoder specified. Using k-dimensionality trees to represent class prototypes.')
    ...
    def score(self, X: np.ndarray, adv_class: int, orig_class: int, eps: float = 1e-10) -> float:
        ...
        if self.enc_model:
            ...
            logger.warning('Need either an encoder or the k-dimensionality trees enabled to compute distance scores.')
    ...
    def attack(self, X: np.ndarray, Y: np.ndarray, target_class: list = None, k: int = None, k_type: str = 'mean',
        ...
        k
            ...
            instances belonging to the class if an encoder is used and to 1 for k-dimensionality trees.
        k_type
        ...
        for _ in range(self.c_steps):
            ...
            for i in range(self.max_iterations):
                ...
                if i % log_every == 0 and self.writer is not None:
                    ...
                    summary.value.add(tag='training/x', simple_value=zt)
