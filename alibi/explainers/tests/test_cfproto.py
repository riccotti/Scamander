...
def tf_keras_iris_explainer(request, models, iris_data):
    ...
    model, ae, enc = models
    if request.param[0]:  # use k-dimensionality trees
        ae = None
        enc = None
...
def test_tf_keras_iris_explainer(disable_tf2, iris_data, tf_keras_iris_explainer, use_kdtree, k):
    ...
    # test fit
    cf.fit(X_train)
    if use_kdtree:  # k-dimensionality trees
        assert len(cf.kdtrees) == cf.classes  # each class has a k-dimensionality tree
        n_by_class = 0
